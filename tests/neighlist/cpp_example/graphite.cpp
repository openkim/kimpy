...
void write_extendxyz(int numberOfParticles,
                     double const * cell,
                     ...
                     int const * speciesCode)
{
  ...
  fs << "Lattice=\"";
  for (int i = 0; i < 9; i++) { fs << " " << cell[i]; }
  fs << "\" Properties=speciesCode:S:1:pos:R:3" << std::endl;
  for (int i = 0; i < numberOfParticles; i++)
  {
    fs << speciesCode[i] << " " << coordinates[i * 3] << " "
       << coordinates[i * 3 + 1] << " " << coordinates[i * 3 + 2] << std::endl;
  }
...
int main()
{
  ...
  double cell[9]
      = {alat, 0, 0, 0.5 * alat, sqrt(3) * 0.5 * alat, 0, 0, 0, 2 * d};
  double * cell1 = cell;
  double * cell2 = cell + 3;
  double * cell3 = cell + 6;
  ...
  {
    coordinates[DIM * i + j]
        = 1 / 3. * cell1[j] + 1 / 3. * cell2[j] + 0 * cell3[j];
  }
  i = 2;
  for (int j = 0; j < 3; j++)
  {
    coordinates[DIM * i + j]
        = 1 / 3. * cell1[j] + 1 / 3. * cell2[j] + 1 / 2. * cell3[j];
  }
  i = 3;
  for (int j = 0; j < 3; j++)
  {
    coordinates[DIM * i + j]
        = 2 / 3. * cell1[j] + 2 / 3. * cell2[j] + 1 / 2. * cell3[j];
  }
  ...
  std::memcpy(
      coords_all, coordinates, sizeof(double) * numberOfParticles * DIM);
  std::memcpy(coords_all + numberOfParticles * DIM,
              coordinatesOfPaddings.data(),
              sizeof(double) * numberOfPaddings * DIM);
  std::memcpy(code_all, speciesCode, sizeof(int) * numberOfParticles);
  std::memcpy(code_all + numberOfParticles,
              speciesCodeOfPadding.data(),
              sizeof(int) * numberOfPaddings);


  /* generate neighborlist */
  int * needNeighbors = new int[total];
  for (int i = 0; i < numberOfParticles; i++) { needNeighbors[i] = 1; }
  for (int i = numberOfParticles; i < total; i++) { needNeighbors[i] = 0; }
  ...
  {
    nbl_build(
        nl, total, coords_all, influenceDistance, 2, cutoffs, needNeighbors);
    ...
    {
      std::cout << "i = " << i << ", neigh = " << neighborsOfParticle[i]
                << std::endl;
    }
