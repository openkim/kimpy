...
def names_for_collection(collection, col):
    for it in [kimpy.collection_item_type.modelDriver,
                      kimpy.collection_item_type.portableModel,
                      kimpy.collection_item_type.simulatorModel]:
        extent, error = col.cache_list_of_item_names_by_collection_and_type(collection, it)
        check_error(error, 'cache_list_of_directory_names')
        print(str(collection), ':', str(it), ':')
        for i in range(extent):
...
def example_main():
    ...
    for kc in [kimpy.collection.system, kimpy.collection.user,
        kimpy.collection.environmentVariable, kimpy.collection.currentWorkingDirectory]:
    ...
    filename, collection, error = col.get_item_library_file_name_and_collection(
        kimpy.collection_item_type.simulatorModel, 'Sim_LAMMPS_LJcut_AkersonElliott_Alchemy_PbAu')
    check_error(error, 'get_item_library_file_name_and_collection')
    print('Simulator Model Sim_LAMMPS_LJcut_AkersonElliott_Alchemy_PbAu has library name ',
    ...
    for i in range(extent):
        file_name, file_length, file_raw_data, avail_as_str, file_str, error = col.get_item_metadata_file(i)
