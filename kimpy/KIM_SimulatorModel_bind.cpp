...
PYBIND11_MODULE(simulator_model, module) {
  ...
  py::class_<SimulatorModel, std::unique_ptr<SimulatorModel, py::nodelete>> cl (module, "SimulatorModel");

  // python constructor needs to return a pointer to the C++ instance
  cl.def(py::init(
    [](std::string const & simulatorModelName, py::array_t<int> error) {
      SimulatorModel * mo;
  ...
  .def("get_simulator_name_and_version",
    [](SimulatorModel& self) {
  ...
  .def("get_number_of_supported_species",
    [](SimulatorModel& self) {
  ...
  .def("open_and_initialize_template_map", &SimulatorModel::OpenAndInitializeTemplateMap)
  ...
  .def("get_number_of_simulator_fields",
    [](SimulatorModel& self) {
  ...
  .def("get_simulator_field_metadata",
    [](SimulatorModel& self, int const fieldIndex) {
      ...
      int error = self.GetSimulatorFieldMetadata(fieldIndex, &extent, &fieldName);
  ...
  .def("get_simulator_field_line",
    [](SimulatorModel& self, int const fieldIndex,int const lineIndex) {
      ...
      int error = self.GetSimulatorFieldLine(fieldIndex, lineIndex, &lineValue);
  ...
  .def("get_parameter_file_directory_name",
    [](SimulatorModel& self) {
  ...
  .def("get_specification_file_name",
    [](SimulatorModel& self) {
  ...
  .def("get_number_of_parameter_files",
    [](SimulatorModel& self) {
