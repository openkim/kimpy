...
PYBIND11_MODULE(collections, module) {
  ...
  py::class_<Collections, std::unique_ptr<Collections, py::nodelete>> cl (module, "Collections");
  ...
  .def("get_item_library_file_name_and_collection",
    [](Collections& self, CollectionItemType const itemType, std::string const & itemName) {
      ...
      int error = self.GetItemLibraryFileNameAndCollection(itemType, itemName, &fileName, &collection);
  ...
  .def("get_item_metadata_file",
    [](Collections& self, int const index) {
      ...
      int error = self.GetItemMetadataFile(index, &fileName, &fileLength,
          &fileRawData, &availableAsString, &fileString);
  ...
  .def("cache_list_of_item_names_by_collection_and_type",
    [](Collections& self, Collection const collection, CollectionItemType const itemType) {
      int extent;
      int error = self.CacheListOfItemNamesByCollectionAndType(collection, itemType, &extent);
  ...
  .def("get_item_library_file_name_by_collection_and_type",
    [](Collections& self, Collection const collection, CollectionItemType const itemType,
        std::string const & itemName) {
      std::string const * fileName;
      int error = self.GetItemLibraryFileNameByCollectionAndType(collection,
          itemType, itemName, &fileName);
  ...
  .def("get_item_metadata_file_by_collection_and_type",
    [](Collections& self, int const index) {
      ...
      int availableAsString;
      std::string const * fileString;
      int error = self.GetItemMetadataFileByCollectionAndType(index, &fileName,
          &fileLength, &fileRawData, &availableAsString, &fileString);
  ...
  .def("cache_list_of_directory_names",
    [](Collections& self, Collection const collection, CollectionItemType const itemType) {
      int extent;
      int error = self.CacheListOfDirectoryNames(collection, itemType, &extent);
