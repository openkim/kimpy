...
PYBIND11_MODULE(log, module) {
  ...
  // self-defined class. If we do that, pybind11 will report argument type matching
  // error.
  .def("log_entry",
    [](Log & self, LogVerbosity const logVerbosity, std::string const & message,
        int const lineNumber, std::string const & fileName) {
      self.LogEntry(logVerbosity, message, lineNumber, fileName);
    }
  )
  .def("log_entry",
    [](Log & self, LogVerbosity const logVerbosity, std::stringstream const & message,
        int const lineNumber, std::string const & fileName) {
      self.LogEntry(logVerbosity, message, lineNumber, fileName);
    }
  );
